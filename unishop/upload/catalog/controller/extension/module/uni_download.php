	...
	public function index($product_id = 0) {
		...
		if($status) {
			...
			if(!$only_customer || ($only_customer && $this->customer->isLogged())) {
				...
				$product_id = isset($this->request->get['product_id']) ? (int)$this->request->get['product_id'] : (int)$product_id;
					...
					foreach ($results as $result) {
						if (file_exists(DIR_DOWNLOAD . $result['filename'])) {
							...
							$data['downloads'][] = [
								...
								'icon'	=> $icon,
								'href'	=> $this->url->link('extension/module/uni_download/download', 'pid='.$product_id.'&did='.$result['download_id'], true)
							];
						}
					}
				}
