<?php
class ControllerProductUniLatest extends Controller {
	public function index() {
		$uniset = $this->config->get('config_unishop2');
		$lang_id = $this->config->get('config_language_id');
		
		$this->load->language('product/uni_latest');
		
		$settings = $uniset['catalog']['latest_page'];
		...
		if(isset($settings['status'])) {
			...
			$this->document->addLink($this->url->link('product/uni_latest', ''), 'canonical');
			...
			$data['breadcrumbs'][] = [
				'text' => $page_heading,
				'href' => $this->url->link('product/uni_latest')
			];
			...
			$filter_data = [
				...
				'order'			=> $order,
				'start'         => 0,
				'limit'         => $settings['limit'] ? (int)$settings['limit'] : 12,
				'date'			=> $settings['date_end'] ? (int)$settings['date_end'] : 10,
			];

			$results = $this->model_extension_module_uni_latest->getProducts($filter_data);
			...
			if($results) {
				...
				if(isset($settings['product_category'])) {
					...
					$categories = $this->model_extension_module_uni_latest->getProductCategories($filter_data);
					...
					if($categories) {
						foreach ($categories as $category) {
							$data['categories'][$category['category_id']] = [
								...
								'href'   		=> $this->url->link('product/uni_latest', '&cat_id='.(int)$category['category_id'] . $url, true)
							];
						}
				...
				foreach($results  as $result) {
					...
					if(isset($new_data['stickers']['new'])) {
			...
			$data['sorts'][] = [
				...
				'href'  => $this->url->link('product/uni_latest', '' . $url)
			];

			$data['sorts'][] = [
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'pd.name-ASC',
				'href'  => $this->url->link('product/uni_latest', 'sort=pd.name&order=ASC' . $url)
			];

			$data['sorts'][] = [
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'pd.name-DESC',
				'href'  => $this->url->link('product/uni_latest', 'sort=pd.name&order=DESC' . $url)
			];

			$data['sorts'][] = [
				'text'  => $this->language->get('text_price_asc'),
				'value' => 'p.price-ASC',
				'href'  => $this->url->link('product/uni_latest', 'sort=p.price&order=ASC' . $url)
			];

			$data['sorts'][] = [
				'text'  => $this->language->get('text_price_desc'),
				'value' => 'p.price-DESC',
				'href'  => $this->url->link('product/uni_latest', 'sort=p.price&order=DESC' . $url)
			];
			...
			if ($this->config->get('config_review_status')) {
				$data['sorts'][] = [
					...
					'value' => 'rating-DESC',
					'href'  => $this->url->link('product/uni_latest', 'sort=rating&order=DESC' . $url)
				];

				$data['sorts'][] = [
					'text'  => $this->language->get('text_rating_asc'),
					'value' => 'rating-ASC',
					'href'  => $this->url->link('product/uni_latest', 'sort=rating&order=ASC' . $url)
				];
			}
			...
			$data['sorts'][] = [
				...
				'value' => 'p.model-ASC',
				'href'  => $this->url->link('product/uni_latest', 'sort=p.model&order=ASC' . $url)
			];

			$data['sorts'][] = [
				'text'  => $this->language->get('text_model_desc'),
				'value' => 'p.model-DESC',
				'href'  => $this->url->link('product/uni_latest', 'sort=p.model&order=DESC' . $url)
			];
			...
			$this->response->setOutput($this->load->view('product/uni_latest', $data));
		} else {
			$data['breadcrumbs'] = [];
