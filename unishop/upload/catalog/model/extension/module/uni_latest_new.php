...
class ModelExtensionModuleUniLatest extends Model {
	...
	public function getProducts($data) {
		...
		$products = $this->getProductsId($data);
		
		if($products) {
			$sql = "SELECT p.product_id, (SELECT AVG(rating) FROM `".DB_PREFIX."review` r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating";
			...
			$sql .= " WHERE p.product_id IN (".implode(',', array_map('intval', $products)).") AND p.status = '1' AND p2s.store_id = '".$store_id."' AND pd.language_id = '".$lang_id."'";
	...
	public function getProductsId($data) {
		...
		$cache_name = 'product.unishop.latest.'.$store_id;
