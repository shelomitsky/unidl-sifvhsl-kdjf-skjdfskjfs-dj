...
class ControllerExtensionModuleUniNews extends Controller {
	...
	private function getList() {
		...
		$data['setting'] = $this->config->get('uni_news') ? $this->config->get('uni_news') : [];
	...
	public function setting() {	
		...
		$data['uni_news'] = $this->config->get('uni_news') ? $this->config->get('uni_news') : [];
		...
		$sitemap_seo_url = $this->model_extension_module_uni_news->getNewsSitemapSeoUrls(0);
		
		$data['sitemaps'] = [];

		$data['sitemaps'][] = [
			'name'     => $this->config->get('config_name'),
			'url'      => ($this->config->get('config_secure') ? HTTPS_CATALOG : HTTP_CATALOG).($sitemap_seo_url ? $sitemap_seo_url : 'index.php?route=extension/feed/uni_news_sitemap')
		];

		$results = $this->model_setting_store->getStores();

		foreach ($results as $result) {
			$sitemap_seo_url = $this->model_extension_module_uni_news->getNewsSitemapSeoUrls($result['store_id']);
			
			$data['sitemaps'][] = [
				'name'     => $result['name'],
				'url'      => $result['ssl'].($sitemap_seo_url ? $sitemap_seo_url : 'index.php?route=extension/feed/uni_news_sitemap')
			];
		}
